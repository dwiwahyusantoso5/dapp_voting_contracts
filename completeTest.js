...
contract("eVote", async (accounts) => {
  ...
  it("Throw an error if elligable user provides invalid DL proof to vote", async () => {
    ...
    try {
      ...
      assert(
        String(err).includes("Invalid DL proof"),
        "error in verifying invalid user"
      );
    }
    await revertToSnapshot(snapshotId);
  ...
  it("Malicious Administrator", async () => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (i == 2) {
        tempComputationArray.push(
          data[i].c.add(tempComputationArray[i - 1]).mul(2)
        );
      } else
    ...
    beginDispute = (
      await eVoteInstance.finishTallyBlockNumber.call()
    ).toNumber();
    ...
    tx = await eVoteInstance.disputeTallyResult(t1, t2, proof2, proof2, {
      from: accounts[1],
    });
    ...
    tx = await eVoteInstance.disputeTallyResult(t1, t2, proof1, proof2, {
      from: accounts[1],
    });
    ...
    //dispute case 3
    proof2 = computationMerkleTree.getHexProof(
      computationArray[data.length - 1]
    );
    t1 = [0, 0, 0];
  ...
  it("Refund deposits for all", async () => {
    beginRefund = (
      await eVoteInstance.finishChallengeBlockNumber.call()
    ).toNumber();
  ...
  it("Hypothetical RegisterVote", async () => {
    ...
    gas = await inst.verifyMerkleProof.estimateGas(
      hypo,
      hypo[hypo.length - 1],
      hypo[0]
    );
