...
function create1outof2ZKPNoVote(m, r, H, i) {
  ...
  if (d1.isNeg()) d1 = d1.add(group.n);
  // calculate r1
  r1 = w.sub(r.mul(d1)).mod(group.n);
  if (r1.isNeg()) r1 = r1.add(group.n);
...
function create1outof2ZKPYesVote(m, r, H, i) {
  ...
  if (d2.isNeg()) d2 = d2.add(group.n);

  // calculate r2
  r2 = w.sub(r.mul(d2)).mod(group.n);
  if (r2.isNeg()) r2 = r2.add(group.n);
...
function genVotingKeyAndProofs() {
...
function genTestData(length) {
  res = [];
  for (i = 0; i < length; i++) {
    res.push(genVotingKeyAndProofs());
  }
...
// vr = data[0];
// console.log(
//   `["${vr.c.getX().toString()}","${vr.c
//     .getY()
//     .toString()}"],["${vr.Y.getX().toString()}","${vr.Y.getX().toString()}"],["${
//     vr.proof01[0]
//   }","${vr.proof01[1]}","${vr.proof01[2]}","${vr.proof01[3]}","${
//     vr.proof01[4]
//   }","${vr.proof01[5]}","${vr.proof01[6]}"]`
// );
// for (i = 0; i < 5; i++) {
//   sum += data[i].v;
